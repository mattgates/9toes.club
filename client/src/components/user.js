...
class User extends React.Component {
    ...
    getRecipes = async () => {
        const url = '/api/posts/recipes?username=' + this.props.match.params.username;
                ...
                <ul>
                    {this.state._id.map((item, i) => (
                        <NavLink to={`/users/${this.state.submittedBy[i]}/${this.state._id[i]}`} key={item}>
                            <li>{this.state.title[i]} ({this.state.type[i]}) by {this.state.author[i]} submitted by {this.state.submittedBy[i]}</li>
                        </NavLink>
                    ))}
                </ul>
            </div>
